...
public class PrecisionTargetAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int viewType) {
        View view;
        if (viewType == 0) {
            view = LayoutInflater.from(context).inflate(R.layout.list_item_precision_head, viewGroup, false);
            return new HeadViewHolder(view);
        } else {
            view = LayoutInflater.from(context).inflate(R.layout.list_item_precision, viewGroup, false);
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder viewHolder, int position) {
        if (viewHolder instanceof ViewHolder && dataList.size() > 0) {
            ...
            ((ViewHolder) viewHolder).positionTv.setText(dataList.get(position).getPrecisionRingNumber());
